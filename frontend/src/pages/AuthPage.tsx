...
export const AuthPage: React.FC = () => {
  ...
  const handleLogin = async (credentials: { email: string; password: string }) => {
    ...
    try {
      const response = await fetch(`http://45.144.221.227:3001/api/auth/login`, {
        method: 'POST',
        headers: {
      ...
      if (result.success) {
        ...
        try {
          const userResponse = await fetch('http://45.144.221.227:3001/api/user/me', {
            headers: {
  ...
  const handleRegister = async (userData: {
    ...
    try {
      const response = await fetch(`http://45.144.221.227:3001/api/auth/register`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
